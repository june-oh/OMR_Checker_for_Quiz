...
스캔 이미지 (JPG/PNG)
    │
    ▼
[4방향 마커 검출] 0°, 90°, 180°, 270° 회전 시도
    │
    ▼
[마커 기반 방향 결정] 왼쪽 위, 오른쪽 위, 오른쪽 아래 마커 위치 확인
    │
    ▼
[이미지 회전] 올바른 방향으로 회전
    │
...
1. **원본 이미지**: 업로드된 원본 스캔 이미지
2. **회전 보정**: 마커 위치 기반 자동 회전 (0°, 90°, 180°, 270° 중 최적 선택)
3. **마커 검출**: 3개 코너 마커 검출 및 표시 (왼쪽 위, 오른쪽 위, 오른쪽 아래)
4. **Affine 변환**: 마커 기준 정렬 및 크롭 → 3507×2480 표준화
5. **컬러 드롭아웃**: 빨간색 인쇄 제거, 검정 마킹만 보존
6. **전처리 완료**: Levels (대비 강화) + GaussianBlur (노이즈 제거)
7. **템플릿 버블 위치**: 인식할 버블 위치를 녹색 사각형으로 표시 (디버깅용)
8. **최종 인식 결과**: 버블 인식 및 마킹 표시, 정답/오답 하이라이트

이를 통해 인식 실패 시 어느 단계에서 문제가 발생했는지 정확히 파악할 수 있습니다.
