...
스캔 이미지 (JPG/PNG - 컬러)
    │
    ▼
[컬러 드롭아웃] max(B,G,R) → 빨간 인쇄 제거, 검정 마킹만 보존 (흑백 변환)
    │
    ▼
[4방향 마커 검출] 0°, 90°, 180°, 270° 회전 시도, 마커 위치 분석
    │
    ▼
[마커 기반 회전] 올바른 방향으로 자동 회전
    │
    ▼
[마커 기반 Affine 변환] 3개 코너 마커의 바깥쪽 기준 → 정렬 + 크롭 → 3507×2480
    │
    ▼
...
2. **컬러 드롭아웃**: max(B,G,R) 기법으로 빨간 인쇄 제거, 흑백 변환
3. **회전 보정**: 마커 위치 기반 자동 회전 (0°, 90°, 180°, 270° 중 최적 선택)
